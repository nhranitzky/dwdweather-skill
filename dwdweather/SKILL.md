...
Default to `--output json` for all CLI calls. Read the JSON and answer the user in concise natural language. Only show raw JSON when the user explicitly asks for JSON, structured data, debugging details, or machine-readable output.
...
```bash
${HERMES_SKILL_DIR}/bin/dwdweather forecast Berlin --days 2 --tz UTC --output json
```
...
```bash
${HERMES_SKILL_DIR}/bin/dwdweather summary LOCATION --days 5 --output json
```

Use `current` for current observed conditions:

```bash
${HERMES_SKILL_DIR}/bin/dwdweather current LOCATION --output json
```
...
```bash
${HERMES_SKILL_DIR}/bin/dwdweather forecast LOCATION --days 3 --output json
${HERMES_SKILL_DIR}/bin/dwdweather forecast LOCATION --days 7 --daily --output json
```
...
```bash
${HERMES_SKILL_DIR}/bin/dwdweather history LOCATION --date 2026-05-01 --output json
${HERMES_SKILL_DIR}/bin/dwdweather history LOCATION --date 2026-05-01 --end-date 2026-05-07 --daily --output json
```

Use `alerts` for active DWD weather warnings:

```bash
${HERMES_SKILL_DIR}/bin/dwdweather alerts LOCATION --output json
```
...
```bash
${HERMES_SKILL_DIR}/bin/dwdweather stations LOCATION --radius 50 --limit 15 --output json
```
...
```bash
${HERMES_SKILL_DIR}/bin/dwdweather summary Freiburg im Breisgau --output json
${HERMES_SKILL_DIR}/bin/dwdweather summary "Freiburg im Breisgau" --output json
```
...
Handled JSON errors are returned as:
